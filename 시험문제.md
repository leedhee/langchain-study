...
class MedicalService:
    def __init__(self):
		    # 1. checkpointer 만들기만 하면 안되고, create_agent에 전달해야 함 
		    # 2. v1 공식 문서에는 InMemorySaver 사용 권장
        self.checkpointer = InMemorySaver()
```
**Bug 2**: 
```python
# 독립함수로 tool 정의하여 agent에서 사용하는 방식 권장
# MedicalService.py
class MedicalService:
...
class MedicalService:
	  def create(self, model):
        return create_agent(
            ...
            # 공식 파라미터명 오타
            response_format=ToolStrategy(ChatResponse),
            # Bug1 - checkpointer 전달 
            checkpointer=medical_service.checkpointer
        )
```
...
**문제점**: - AgentService의 process_query가 실행될 때마다 checkpointer가 갱신되므로, 동일한 thread_id 기반 대화의 연속성이 깨지게 된다.
           - 따라서, InMemorySaver()를 매 요청마다 생성하지 말고, 서비스 초기화 시 한 번만 생성해야 한다.
**수정 코드**:
```python
class AgentService:
    def __init__(self):
...
**(1)** 상태: v1 문서 기준 맞음 
**(2)** 상태: v1 문서 기준 맞음 
**(3)** 상태: v1 문서 기준 from langchain.messages import HumanMessage 권장
**(4)** 상태: v1 문서 기준 맞음
**(5)** 상태: v1 문서 기준 from langchain.tools import tool 권장
...
**답변** (3~5문장):
- response_format=ToolStrategy(ChatResponse)는 LangChain이 공식적으로 제공하는 구조화 출력 방식으로, 에이전트의 최종 응답을 ChatResponse 스키마에 맞게 생성하고 검증한 뒤 반환한다.
- 반면, ChatResponse를 @tool로 직접 정의하면, 이는 최종 응답 형식을 강제하는 장치가 아니라, LLM이 필요 시 호출하는 일반 도구로 취급된다.
- 또한, @tool 방식은 응답이 항상 해당 Pydantic 모델 형태를 따르도록 보장하지 못한다.
- 따라서 공식 문서 기준 response_format=ToolStrategy(ChatResponse) 방식이 권장되며, 프레임워크 차원에서 스키마 기반 검증과 일관된 반환 형식을 제공할 수 있다는 장점이 있다.
